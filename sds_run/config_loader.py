...
def load_config(config_path: str = 'config.yaml') -> Dict:
    if not os.path.exists(config_path):
        raise FileNotFoundError(f'Error: The file {config_path} does not exists')
    try:
        with open(config_path, 'r') as f:
            config = yaml.safe_load(f)
    except yaml.YAMLError as e:
        print(f'Error to look at YAML file: {e}')
        raise
    if not config:
        raise ValueError(f'Error: The config file {config_path} is empty')
    #verifting if the keys for the aplication are there:
